...
def plot(name, enrichment_df):
    ...
    distances = [v for v in distance_dict.values() if not pd.isna(v)]
...
for name, enrichment_df in dfs.items():
    plot(name, enrichment_df)

order_lock_dfs = {"locked_noctcf_cis_enrichment_matrix": dfs['noctcf_cis_enrichment_matrix'],
                "locked_noctcf_trans_enrichment_matrix": dfs['noctcf_trans_enrichment_matrix'],
                "locked_geq25k_noctcf_cis_enrichment_matrix": dfs['geq25k_noctcf_cis_enrichment_matrix'],
                "locked_geq25k_noctcf_cis_enrichment_matrix": dfs['geq25k_noctcf_trans_enrichment_matrix'],
                "locked_geq50k_noctcf_cis_enrichment_matrix": dfs['geq50k_noctcf_cis_enrichment_matrix'],
                "locked_geq50k_noctcf_trans_enrichment_matrix": dfs['geq50k_noctcf_trans_enrichment_matrix'],
                "locked_geq25k_cis_enrichment_matrix": dfs['geq25k_cis_enrichment_matrix'],
                "locked_geq25k_cis_enrichment_matrix": dfs['geq25k_trans_enrichment_matrix'],}

for name, enrichment_df in order_lock_dfs.items():
    max = enrichment_df.max().max()
    log10_enrichment_df = np.log10(enrichment_df.replace(0, 10 ** (-np.log10(max))))
